...
class NetvarsManager:
    def __init__(self):
        ...
        """
        client_handle = module_from_name(pm.process_handle, 'client.dll'
        )
        client_bytes = pm.read_bytes(client_handle.lpBaseOfDll, client_handle.SizeOfImage)
    ...
    def dump_netvars(self, out_file=sys.stdout, json_format=False, printout=False) -> None:
...
NetvarsManager().dump_netvars(json_format=True)